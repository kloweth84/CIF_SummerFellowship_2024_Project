---
title: "Survey Communication Analysis"
subtitle: "Code Sample for IDA by Katharyn Loweth"
date: "2025-02-27"
output:
...
Background:  The sample code was pulled from engagement analysis of communication strategies completed as part of data analyst fellowship at the U.S. Census Bureau in June-August 2024. The code specifically analyzes how respondent engagement changes in the time period after planned survey mailings were sent to non-respondents for 5 different establishment-level surveys across 2017-2022 time period. 

Disclaimer: Any opinions and conclusions expressed herein are those of the author(s) and do not reflect the views of the U.S. Census Bureau. The Census Bureau has reviewed this data product to ensure appropriate access, use, and disclosure avoidance protection of the confidential \ CBDRB-FY24-EWD001-007)

The database connection has been changed for external publication. For purposes of code example, setting the eval=FALSE globally so that all code can be viewed in the rendered document. 

```{r setup, include=FALSE}
knitr::opts_chunk$set(tidy.opts = list(width.cutoff = 60), tidy = TRUE, echo = TRUE, eval = FALSE)
```
...
Previous manipulation occurred for the survey data (in exploratory analysis modeling file) that calculated the daily authentication code burn use and ratio for each day in the collection period where code use is greater than 0. 
...
mail_window = function(surveydata, maildata, surveyname, windowlimit){
  ...
  for (i in seq(nrow(surveymaildata))){
    ...
    #creates a window of time, because maildate is a date datatype adding a number will result in a date in the future
    mailwindow = maildate + windowlimit 
    ...
    #if statement excludes instances where no codes were used in the period following the mailout date
    if (nrow(filtered_surveydata) >0){
      filtered_length = nrow(filtered_surveydata)-1
      filtered_surveydata$DOW = weekdays(filtered_surveydata$AUTHDATE_BANK)
      #counts the number of days within the window; day 0 is the mailout day
      day_count = data.frame("Day_Count" = seq(0, filtered_length)) 
      #represents the count of the specific mailing in the surveymaildata
      mail_count = data.frame("Mail_Count" = rep(i, nrow(filtered_surveydata))) 
...
percent_change = function(dataset){
  ...
  #replacing values for the mailout day since we are not looking outside the mailing window 
  d2$Percent_Change[d2$Day_Count == 0] <- NA 
  return(d2)
}
...
## Median Burn Count Visualization 
```{r}
...
mail_window = ggplot(data = median_mail_count, aes(x =Day_Count, y = median_usecount, color = Survey)) + geom_line(linewidth = 1)  + scale_color_manual(values = cols) + theme_minimal() 
+ ggtitle("Impact of Mailings Occurs 4-7 Days after Sendout") 
+ theme(text = element_text(size = 14)) + ylab("Median Number of Authentication Codes Burned") 
+ scale_x_continuous(name = "", limits=c(0, 10), breaks=c(0,5,10), label = c("Mailout Day", "5 Days After", "10 Days After"))
mail_window
...
mail_window_change = ggplot(data = median_mail_count, aes(x =Day_Count, y = median_percchange, color = Survey)) + geom_line(linewidth = 0.75)  + scale_color_manual(values = cols) + theme_minimal() 
+ ggtitle("Authentication Code Use Percentage Change the 14 days after Mailings", 
          subtitle = "For 5 Annual Economic Surveys") + theme(text = element_text(size = 10, family = "Roboto")) + ylab("Median Authentication Code Use Count") + xlab("Days since Mailing")
mail_window_change
...
visuals looking at mailing periods and highlights if on that day an email was sent
